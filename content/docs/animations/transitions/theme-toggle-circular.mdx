...
The `ThemeToggleCircular` component creates an expanding circle effect from the click position when toggling themes. Uses the native View Transitions API for smooth, hardware-accelerated animations.
...
```css
::view-transition-new(root) {
  animation: circle 0.5s cubic-bezier(0.68, -0.55, 0.265, 1.55);
}
```
